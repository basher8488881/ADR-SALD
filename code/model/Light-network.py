...
class ResnetPoolBlockConv1d(nn.Module):
    ...
    def __init__(self, size_in, size_h=None, size_out=None, shortcut=True, atten=False):
        ...
        #self.bn_01 = nn.BatchNorm1d(size_h)
...
class ResnetPointnet(nn.Module):
    ...
    def __init__(self, c_dim=128, dim=3, hidden_dim=128, normal_channel=False):
        super().__init__()
        self.c_dim = c_dim
...
class ImplicitMap_fExtrator(nn.Module):
    def __init__(
        ...
        self.base_nf = 384
        self.d_conv1d_1 = Conv1dGrad(131, 515, kernel_size=1)

        self.d_conv1d_10 = Conv1dGrad(576, 1, kernel_size=1)
        ...
        torch.nn.init.normal_(self.d_conv1d_10.weight, mean=np.sqrt(np.pi) / np.sqrt(576), std=0.0001)
        ...
        self.ResNextFExtractor1 = ResNext_fExtractor(in_channel=515, filter_list=[16,16,16,16,16,16,16,16,16,16,16,16,16,16,16,16])
        self.ResNextFExtractor1_1 = ResNext_fExtractor(in_channel= 256, filter_list=[24,24,24,24,24,24,24,24,24,24,24,24,24,24,24,24])
        self.ResNextFExtractor2 = ResNext_fExtractor(in_channel=384, filter_list=[32,32,32,32,32,32,32,32,32,32,32,32])
        self.ResNextFExtractor2_2 = ResNext_fExtractor(in_channel=384, filter_list=[40,40,40,40,40,40,40,40,40,40,40,40])
        self.ResNextFExtractor3 = ResNext_fExtractor(in_channel=480, filter_list=[48,48,48,48,48,48,48,48,48,48,48,48])
        self.ResNextFExtractor3_3 = ResNext_fExtractor(in_channel=576, filter_list=[56,56,56,56,56,56,56,56])
        self.ResNextFExtractor4 = ResNext_fExtractor(in_channel=448, filter_list=[64,64,64,64,64,64])
        self.ResNextFExtractor4_4 = ResNext_fExtractor(in_channel=384, filter_list=[128, 128, 128, 128,128,128])
        self.ResNextFExtractor5 = ResNext_fExtractor(in_channel=768, filter_list=[136,136,136,136,136])
        self.ResNextFExtractor5_5 = ResNext_fExtractor(in_channel=680, filter_list=[144,144,144,144])
...
class Network(nn.Module):
    def __init__(self, conf, latent_size, auto_decoder):
        ...
        self.encoder = ResnetPointnet(hidden_dim= 128, c_dim=128, dim=encoder_input_size,normal_channel=False) if not auto_decoder and latent_size > 0 else None
    ...
    def forward(self, manifold_points, manifold_normals, sample_nonmnfld, latent, 
        ...
        if self.encoder is not None and not only_decoder_forward:
            ...
            q_latent_mean, q_latent_std = self.encoder( encoder_input) #, nfeature

            q_z = dist.Normal(q_latent_mean, (torch.exp(q_latent_std)))
            latent = q_z.rsample() #+ q_latent_mean
