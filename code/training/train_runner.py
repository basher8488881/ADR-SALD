        ...
        for epoch in range(self.start_epoch, self.nepochs + 2):
            ...
            if epoch % self.conf.get_int('train.plot_frequency') == 0 and epoch > 0:
                ...
                for i in range(min(1, self.ds_len)):
                    ...
                    #filename = '{0}/uniform_iteration_{1}_{2}'.format(self.plots_dir, epoch, i) 
                    #filename = filename + '.ply'
                    #mesh = extract_fields(resolution=100,  query_func=self.network, points=pnts, latent=latent, out_dir=filename, iter_step=epoch) 
            ...
            data_index = 0
            for pnts_mnfld,normals_mnfld,sample_nonmnfld,scale_param,indices in self.dataloader:
