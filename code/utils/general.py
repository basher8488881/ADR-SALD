...
def compute_trimesh_chamfer(gt_points, gen_mesh, offset, scale, num_mesh_samples=30000):
    ...
    gt_points_np = gt_points.reshape(-1,3) #.vertices
    ...
    # recon_kd_tree = KDTree(gen_points_sampled)
    # gt_kd_tree = KDTree(gt_points_np)
    # re2gt_distances, re2gt_vertex_ids = recon_kd_tree.query(gt_points)
    # gt2re_distances, gt2re_vertex_ids = gt_kd_tree.query(gen_points_sampled)
    ...
    # pcd_rec.points = o3d.utility.Vector3dVector(gen_points_sampled)

    # o3d.io.write_point_cloud(os.path.join('/media/digiaires/2TBSSD1/ADR-SALD-Extra/ADR-SALD/', 'rec_points.ply'), pcd_rec)

    # pcd_gt = o3d.geometry.PointCloud()
    # pcd_gt.points = o3d.utility.Vector3dVector(gt_points_np)

    # o3d.io.write_point_cloud(os.path.join('/media/digiaires/2TBSSD1/ADR-SALD-Extra/ADR-SALD/', 'gt_points.ply'), pcd_gt)    

    return {'gt_to_gen_chamfer':gt_to_gen_chamfer, 'gen_to_gt_chamfer': gen_to_gt_chamfer}
    #return {'gt_to_gen_chamfer':chamfer_dist, 'gen_to_gt_chamfer': hausdorff_distance} 

def compute_dists(gt_points, recon_points):
    #print(gt_points)
    ...
    chamfer_dist = 0.5* (cd_re2gt + cd_gt2re)
    hausdorff_distance = np.max((hd_re2gt, hd_gt2re))
    #print(chamfer_dist, hausdorff_distance)
    return {'gt_to_gen_chamfer':chamfer_dist, 'gen_to_gt_chamfer': hausdorff_distance} 
