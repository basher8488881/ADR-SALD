...
# from utils.surface_extraction import as_mesh, surface_extraction

# def extract_fields( resolution, query_func, points, latent, out_dir, iter_step):
#     N = 16
#     points = points.squeeze().detach().cpu().numpy()
#     bound_min = np.array([np.min(points[:,0]), np.min(points[:,1]), np.min(points[:,2])]) -0.05
#     bound_max = np.array([np.max(points[:,0]), np.max(points[:,1]), np.max(points[:,2])]) +0.05
...
#                 # grad = grad_func.gradient(pts, points).reshape(len(xs), len(ys), len(zs), 3).detach().cpu().numpy()
#                 # val = query_func.udf(pts, points).reshape(len(xs), len(ys), len(zs)).detach().cpu().numpy()

#                 val, grad, _ = query_func(pts, pts,sample_nonmnfld=None, latent=latent, only_encoder_forward=False, only_decoder_forward=True)
...
#     mesh = surface_extraction(u, g, out_dir, iter_step, bound_min, bound_max, resolution)
#     print(out_dir)
#     mesh.export(out_dir)
#     return mesh
...
def plot_surface(with_points, points, decoder, latent, path, epoch, in_epoch,
    ...
    if (not os.path.exists(filename) or overwrite):
        ...
        for key,val in z_func.items():
            #mesh = extract_fields(resolution, decoder, points, )
